...
file_env 'KEYCLOAK_ADMIN_USERNAME'
file_env 'KEYCLOAK_ADMIN_PASSWORD'

if [[ -n ${KEYCLOAK_ADMIN_USERNAME:-} && -n ${KEYCLOAK_ADMIN_PASSWORD:-} ]]; then
    /opt/jboss/keycloak/bin/add-user-keycloak.sh --user "$KEYCLOAK_ADMIN_USERNAME" --password "$KEYCLOAK_ADMIN_PASSWORD"
fi
...
echo ""
echo " Enabling $DB_NAME Keycloak Database "
echo ""
...
if [ ! -e "$configured_file" ]; then
    ...
    /opt/jboss/tools/provider.sh
    /opt/jboss/tools/theme.sh
