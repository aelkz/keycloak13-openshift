if [ -n "$DATAGRID_HOST" ]; then
    echo ""
    echo "Enabling Red Hat Datagrid communication"
    echo "> Using remote cache at ${DATAGRID_HOST:-localhost} with port: ${DATAGRID_PORT:-11222}"
    echo ""
    $JBOSS_HOME/bin/jboss-cli.sh --file="/opt/jboss/tools/cli/infinispan/remote.cli" >& /dev/null
    $JBOSS_HOME/bin/jboss-cli.sh --file="/opt/jboss/tools/cli/infinispan/remote-metrics.cli" >& /dev/null
fi
