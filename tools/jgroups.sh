...
if [ -n "$JGROUPS_PING_PROTOCOL" ]; then
    ...
    echo "Setting JGroups discovery to $JGROUPS_PING_PROTOCOL with properties $JGROUPS_DISCOVERY_PROPERTIES_PARSED"
    echo "set keycloak_jgroups_discovery_protocol=${JGROUPS_PING_PROTOCOL}" >> "$JBOSS_HOME/bin/.jbossclirc"
    ...
    if [ -f "/opt/jboss/tools/cli/jgroups/discovery/$JGROUPS_PING_PROTOCOL.cli" ]; then
       echo "$JGROUPS_PING_PROTOCOL not found. Using default configuration (default.cli)"
       $JBOSS_HOME/bin/jboss-cli.sh --file="/opt/jboss/tools/cli/jgroups/discovery/$JGROUPS_PING_PROTOCOL.cli" >& /dev/null
    else
